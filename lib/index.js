...
const defaultOpts = { format: 'jpeg', mode: 'good', brick }
...
const render = (opts, src) => {
  ...
  const pixelInterval = opts.pixelInterval || pixelIntervalPreset[opts.mode]
  const getAverageColor = makeGetAverageColor(pixelInterval)
  const renderBrick = makeRenderBrick(brick, getAverageColor, ctx)

  ctx.patternQuality = opts.patternQuality || opts.mode
  ctx.filter = opts.filter || opts.mode
...
export default function legofy (opts = {}) {
  ...
  return duplex(concat(body => render(opts, body).pipe(cat)), cat)
}
