 ...
 * Automatically called by the constructor when `manual` option is not `false`,
...
Legofy.prototype.initialize = function () {
  ...
  this.image = this._loadImage(this.options.input);

  if (this.options.manual !== false) {
    this.process();
    this.output(this.getOutputPath(), this.options.callback);
  }
 ...
 * Automatically called by the `initialize(..)` method when `manual` option is
 * not `false`, otherwise must be called manually.
 * @return {Object} - Self
 */
Legofy.prototype.process = function () {
 ...
 */
Legofy.prototype.output = function (destination = this.getOutputPath(), callback = this.options.callback) {
 ...
 */
Legofy.prototype._loadImage = function (source, callback) {
  let image = new Image();
  image.src = source;
 ...
 */
Legofy.prototype._getAverageColor = function (x, y) {
 ...
 * @param {Number} y      - Starting Y coordinate
 */
Legofy.prototype._drawBrick = function (canvas, x, y) {
  // Get average color for brick from main canvas
  const color = this._getAverageColor(x, y);
